...
- Keep each bullet to one sentence, crisp and action-oriented.
  ...
  return None
  def *sig_stage_weight(sig: str) -> int:
  if sig.startswith(“VALIDATED*”):
  return 4
  ...
  md.append(””)
  md.append(”*Logic: score each ticker by stage (EARLY=1, CONFIRMED=3, VALIDATED=4) × direction (BREAKOUT=+1, BREAKDOWN=-1), then aggregate by sector.*”)
  md.append(””)
...
print(f”[yahoo_quote] error for chunk starting {chunk[0]}: {type(exc).**name**}: {exc}”)
...
if df is None or df.empty:
return f”**{title}:** *None ≥ {MOVER_THRESHOLD_PCT:.0f}%*\n”
t = df.copy()
...
next_date = *to_date(ed[0])
else:
next_date = *to_date(ed)
except Exception:
pass
...
out[“reason”] = f”exception:{type(e).**name**}”
return out
...
alternations = *alternation_count(touch_events)
# Containment (for rectangles quality)
seg_close = c.iloc[start_i : end_i + 1]
inside = 0
total = 0
for j, px in enumerate(seg_close.values, start=start_i):
up = *line_eval(a_u, b_u, float(j)) + tol
lo* = *line_eval(a_l, b_l, float(j)) - tol
if np.isfinite(px) and np.isfinite(up) and np.isfinite(lo*):
total += 1
if lo* <= float(px) <= up:
inside += 1
...
*point_meta(d, best[“trigger_i”], best[“trigger”], “Trigger”),
],
“lines”: [
*line_meta(d, best[“event_low_i”], best[“event_low”], best[“bounce_i”], best[“bounce_high”], “Bounce leg”),
*line_meta(d, best[“event_low_i”], best[“event_low”], len(d) - 1, best[“event_low”], “Conservative trigger”),
*line_meta(d, best[“trigger_i”], best[“trigger”], len(d) - 1, best[“trigger”], “Active trigger”),
],
...
# cand.level is a placeholder; gating uses the dynamic EMA20 via *level_at_bar.
...
Returns (prefix, distance_in_atr), where prefix in {””, “EARLY*”, “CONFIRMED*”}.
“””
...
if price_ok:
return “CONFIRMED*”, float(dist_atr)
if abs(float(close) - float(level)) <= EARLY_MULT * float(atr_val):
return “EARLY*”, float(dist_atr)
...
stage_age_bars = None
breakout_start = None
run_start = *confirm_run_start(cand, d, a)  # (kept for CONFIRMED-only)
# Prefer VALIDATED lifecycle (can remain active even if today is not “fully confirmed”)
vinfo = *validated_stage(cand, d, a, len(d) - 1)
...
prefix, dist_atr = *classify_vs_level(close, level_now, atr_val, cand.direction, vol_ratio, clv)
if prefix != “EARLY*”:
continue
...
return neckline
def *annotate_ihs_dt(ax, dates, close, high) -> Optional[float]:
...
ax.text(dates[-1], neckline, “ Neckline”, va=“bottom”)
return neckline
def *annotate_wedge(ax, dates, high, low, lookback: int = 120) -> None:
“””
...
Works for both WEDGE_UP** and WEDGE_DOWN**.
“””
import numpy as _np
n = len(high)
if n < 40:
...
try:
t1 = _to_ts(ln.get(“t1”)); t2 = *to_ts(ln.get(“t2”))
...
try:
t = *to_ts(pt.get(“t”)); y = float(pt.get(“p”))
